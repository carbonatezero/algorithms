...
class Graph:
    ...
    # Add an edge to self from vertex v to vertex w.
    ...
    def contract(self, v, w):
        for node in self._adj[w]:  # merge the nodes from w to v
             if node != v:  # we dont want to add self-loops
                 self._adj[v].append(node)
                 self._adj[node].append(v)
             self._adj[node].remove(w)  # delete the edges to the absorbed         
        del self._adj[w]  # delete the absorbed vertex 'w'

    def kargerMinCut(self):
        while self.countV() > 2:
             v = random.choice(list(self.vertices()))
             w = random.choice(self._adj[v])
             self.contract(v, w)
        return len(self._adj[list(self.vertices())[0]])
...
def main():
    ...
    cuts = []
    for i in range(n**2):
        cuts.append(graph.kargerMinCut())
        del graph
        graph = Graph(file)
    print(cuts)
    print(min(cuts))
